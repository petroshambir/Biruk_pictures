...
function AdminDashboard() {
  ...
  const fetchPortals = async () => {
    try {
      const res = await fetch('https://biruk-pictures-server.onrender.com/api/client/portals');
      if (res.ok) {
  ...
  const handleCreatePortal = async (e) => {
    ...
    try {
      const res = await fetch('https://biruk-pictures-server.onrender.com/api/client/create-portal', {
        method: 'POST',
  ...
  const handleClientImageUpload = async (e) => {
    ...
    try {
      for (let i = 0; i < files.length; i += BATCH_SIZE) {
        ...
        const res = await fetch('https://biruk-pictures-server.onrender.com/api/client/upload-image', {
          method: 'POST',
          body: formData,
  ...
  const handleDeletePortal = async (id) => {
    ...
    try {
      const res = await fetch(`https://biruk-pictures-server.onrender.com/api/client/delete-portal/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const handleSave = async (title, data) => {
    try {
      ...
      const res = await fetch(`https://biruk-pictures-server.onrender.com/api/projects/${title}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      for (let i = 0; i < files.length; i += BATCH_SIZE) {
        ...
        const res = await fetch(`https://biruk-pictures-server.onrender.com/api/projects/${title}/upload`, {
          method: 'POST',
          body: formData
        });
